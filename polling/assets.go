...
func NewDataSources(conf *viper.Viper, enforce bool) *DataSources {
	...
	staleDuration := d.viperConfig.GetDuration(config.ConfigPollingStaleDuration)
	if staleDuration == 0 {
		staleDuration = time.Minute * 30
	}

	d.staleDuration = staleDuration
...
func (d *DataSources) PullAllPEGAssets(oprversion uint8) (pa PegAssets, err error) {
	...
	for _, asset := range assets {
		...
		// TODO: When V3 is active, we can move this check to into the
		// 		PullBestPrice function
...
func (d *DataSources) PullBestPrice(asset string, reference time.Time, sources map[string]IDataSource) (pa PegItem, err error) {
	...
	// TODO: When V3 is active, we can drop this exception.
	//		All this conditional does it allows PEG to have a 0 value.
	if asset == "PEG" {
		return pa, nil
	}
